...
// GetDirectoryManifest 获取目录清单
func (ds *DirectorySync) GetDirectoryManifest(ctx context.Context) (*DirectoryManifest, error) {
	localFiles, err := ds.scanLocalFiles()
	if err != nil {
		return nil, err
	}
	...
	for path, info := range localFiles {
		manifest.Files[path] = info
	}
	
	return manifest, nil
}

// UploadManifest 上传目录清单
func (ds *DirectorySync) UploadManifest(ctx context.Context, manifest *DirectoryManifest) error {
	data, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return err
	}
	
	manifestPath := ds.remotePath + "/manifest.json"
	return ds.storage.Upload(ctx, manifestPath, data)
}

// DownloadManifest 下载目录清单
func (ds *DirectorySync) DownloadManifest(ctx context.Context) (*DirectoryManifest, error) {
	manifestPath := ds.remotePath + "/manifest.json"
	
	data, err := ds.storage.Download(ctx, manifestPath)
	if err != nil {
		return nil, err
	}
