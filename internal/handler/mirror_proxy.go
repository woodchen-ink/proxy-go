...
func (h *MirrorProxyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// 记录统计信息（缓存未命中）
	collector.RecordRequestWithCache("/mirror", resp.StatusCode, time.Since(startTime), written, iputil.GetClientIP(r), r, false, 0)
}
...
func (h *MirrorProxyHandler) handleCacheHit(w http.ResponseWriter, r *http.Request, item *cache.CacheItem, notModified bool, startTime time.Time, collector *metrics.Collector) {
	...
	if notModified {
		...
		collector.RecordRequestWithCache("/mirror", http.StatusNotModified, time.Since(startTime), 0, iputil.GetClientIP(r), r, true, item.Size)
		return
	}

	http.ServeFile(w, r, item.FilePath)
	// 记录缓存命中，节省的字节数等于文件大小
	collector.RecordRequestWithCache("/mirror", http.StatusOK, time.Since(startTime), item.Size, iputil.GetClientIP(r), r, true, item.Size)
}
